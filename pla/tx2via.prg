...
/*
 ...
 \ Funcao..: Gerenciador do subsistema de taxas a imprimir
 ...
 DO CASE
  CASE op_cad=01                                   // inclus„o
   ...
   IF AT("D",exrot[op_sis])=0                      // se usuario pode fazer inclusao
    TX2_INCL()                                     // neste arquivo chama prg de inclusao
...
PROC TX2_incl     // inclusao no arquivo TX2VIA
...
TX2_TELA()                                         // imp tela para inclusao
 ...
 */
 ...
 cabem=INT((cabem-2048)/TX2VIA->(RECSIZE()))
 ...
 */
 ...
 @ l_s+l_a,c_s+11 GET  tipo;
                  PICT sistema[op_sis,O_CAMPO,02,O_MASC]
                  DEFINICAO 2

 @ l_s+l_a,c_s+13 GET  circ;
 ...
 IF FOUND() .AND. !aprov_reg_                      // pesquisou e achou!
  ...
  DISPBEGIN()
  TX2_GETS()                                       // mostra conteudo do registro
  INFOSIS()                                        // exibe informacao no rodape' da tela
  DISPEND()
  ALERTA()
 ...
 ENDI
  ...
  IF FOUND() .AND. !aprov_reg_                     // por outro usuario, entao
   ...
   FOR i=1 TO FCOU()
    ...
    REPL &msg. WITH M->&msg.
   NEXT
   TX2_GET1(FORM_INVERSA)                          // executa formula inversa
   RECA
   FOR i=1 TO FCOU()
    msg=FIEL(i)
    REPL &msg. WITH rep[i]
 ...
 ELSE                                              // se nao rola a campos para cima
  ...
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+11,l_max-1,c_s+11,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+13,l_max-1,c_s+15,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+17,l_max-1,c_s+51,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+53,l_max-1,c_s+56,1)
 ENDI
ENDD
...
PROC TX2_tela     // tela do arquivo TX2VIA
tem_borda=.t.
...
SETCOLOR(drvcortel)
@ l_s+01,c_s+1 SAY " Codigo ³ Circular"
@ l_s+02,c_s+1 SAY "ÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ"
@ l_s+03,c_s+1 SAY "        ³"
@ l_s+04,c_s+1 SAY "        ³"
@ l_s+05,c_s+1 SAY "        ³"
@ l_s+06,c_s+1 SAY "        ³"
...
PROC TX2_gets     // mostra variaveis do arquivo TX2VIA
...
PROC TX2_get1     // capta variaveis do arquivo TX2VIA
LOCAL getlist := {}
PRIV  blk_tx2via:=.t.
PARA tp_mov
IF tp_mov=INCLUI
