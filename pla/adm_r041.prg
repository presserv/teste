...
rlanc2_=CTOD('01/01/2020')                                   // Lan‡.Final
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
	...
	DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
	 ...
	 IF (R04101F9()) .OR. so_um_reg                  // se atender a condicao...
		...
		@ cl,022 SAY &drvpenf+codigo+&drvtenf  // Codigo
		@ cl,059 SAY TRAN(grupo,"!!")                  // Grupo
		REL_CAB(7)                                     // soma cl/imprime cabecalho
		@ cl,005 SAY "Titular: "+nome                              // Nome
		@ cl,054 SAY "CPF.: "+TRAN(cpf,"@R 999.999.999-99")     // CPF
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,005 SAY "RG.....: "+TRAN(LEFT(rg,13),"@!")            // R.G.
		@ cl,050 SAY "Admissao: "+DTOC(admissao)
		REL_CAB(1)
		@ cl,005 SAY "Enderec: "+endereco                          // Endere‡o
		@ cl,054 SAY "Bair: "+LEFT(bairro,17)                            // Bairro
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,005 SAY "CEP....: "+TRAN(cep,"@R 99999-999")          // CEP
		@ cl,024 SAY "Cidade: "+RTRIM(cidade)+[ ]+TRAN(uf,"!!")			    // Cidade
		@ cl,054 SAY "Fone: "+TRAN(telefone,"@!")               // Telefone
		REL_CAB(2)                                     // soma cl/imprime cabecalho
		@ cl,005 SAY "Nascimento: "+dtoc(nascto_)
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,005 SAY "Naturalid.: "+TRAN(natural,"@!")                // Nascto
		REL_CAB(2)                                     // soma cl/imprime cabecalho
...
//    @ cl,055 SAY LEFT(MEMOLINE(GRUPOS->obs,60,7),20)  //Profissao
		...
		IF !(ECOB->(EOF()))
		 @ cl,005 SAY "OUTROS"
		 REL_CAB(1)
		 @ cl,005 SAY "Enderec: "+ECOB->endereco                    // Endere‡o
		 @ cl,054 SAY "Bair: "+LEFT(ECOB->bairro,17)             // Bairro
		 REL_CAB(1)                                     // soma cl/imprime cabecalho
		 @ cl,005 SAY "CEP....: "+TRAN(ECOB->cep,"@R 99999-999")    // CEP
		 @ cl,024 SAY "Cidade: "+RTRIM(ECOB->cidade)+[ ]+TRAN(ECOB->uf,"!!")		      // Cidade
		 @ cl,054 SAY "Fone: "+TRAN(ECOB->telefone,"@!")         // Telefone
		ENDI
		REL_CAB(2)                                     // soma cl/imprime cabecalho
		@ cl,000 SAY "." //+STR(CL)
		cl:=30
     ...
     @ cl,013 SAY CHR(27)+'3'+chr(15)              // Habilita 216/20
		 ...
		 DO WHIL ! EOF() .AND. chv031=codigo //LEFT(&(INDEXKEY(0)),LEN(chv031))
			 ...
			 IF grau>'1'                                  // se atender a condicao...
			  ...
			  @ cl,053 SAY DTOC(nascto_)
			  @ cl,065 SAY SUBSTR([TitPaiMaeSgoSgaEspFilDep],(VAL(grau)-1)*3+1,3)
			  REL_CAB(2)                                  // soma cl/imprime cabecalho
