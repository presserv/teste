 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.(TAXAS->codigo+TAXAS->tipo+TAXAS->circ=M->mcodaux))
   ...
   IF (so_um_reg.AND.(TAXAS->codigo+TAXAS->tipo+TAXAS->circ=M->mcodaux)).OR.;
    ...
    IMPAC("+------ Contrato -------------+-- Vencimento -------+ : +------ Contrato -------------+-- Vencimento--------+",cl,000)
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    ...
    @ cl,052 SAY "| : |"
    @ cl,060 SAY GRUPOS->grupo+'  '+GRUPOS->codigo // Codigo 3
    @ cl,086 SAY "| Manut.:"
    ...
    @ cl,069 SAY TRAN(TAXAS->emissao_,"@D")   // Vencto 2
    @ cl,086 SAY "| Total.:"
    ...
    @ cl,105 SAY TRAN(GRUPOS->uf,"!!")             // UF
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+---------------------------------------------------+ : +---------------------------------------------------+"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    @ cl,001 SAY LEFT(CIRCULAR->menscirc,50)// Mens1
    @ cl,052 SAY "| : |"
    @ cl,057 SAY LEFT(M->lindeb,50)                // debitos
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    @ cl,001 SAY LEFT(CIRCULAR->menscirc1,50)// Mens2
    @ cl,052 SAY "| : |                                                   |"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    @ cl,001 SAY LEFT(CIRCULAR->menscirc2,50)// Mens 3
    @ cl,052 SAY "| : |"
    @ cl,108 SAY "|"
    IF M->combarra=[S]
     CODBARRAS({{codigo+tipo+circ,4,13,80}},10,6)
    ENDI
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    @ cl,001 SAY LEFT(M->lindeb,50)                // debitos
    @ cl,052 SAY "| : |"
    @ cl,108 SAY "|"
    IF M->combarra=[S]
     CODBARRAS({{codigo+tipo+circ,4,13,80}},10,6)
    ENDI
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    IMPAC("+--------- Autenticacao Mecanica no verso ----------+ : +-",cl,000)
    @ cl,058 SAY TRAN(pg_-1,'9999')                  // n£mero da p gina
    IMPAC("---- Autenticacao Mecanica no verso ----------+",cl,062)
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY REPL(".",109)
// Processamentos
   #ifdef COM_REDE
