...
/*
 ...
 \ Funcao..: Rateio - PM.Goiania - Manut
...
BEGIN SEQUENCE
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
   ...
   M->rcod1:=LPAD(VAL(M->rcod1),6,[0])
   M->rcod2:=LPAD(VAL(M->rcod2),6,[0])
	...
	DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
		...
		ENDI
	 ENDI
   IF codigo < M->rcod1.AND.M->rcod1>[000000]
    SKIP
    LOOP
   ENDI

   IF codigo > M->rcod2.AND.M->rcod2>[000000]
    IF EMPT(cpord)
     GO BOTT
    ENDI
    SKIP
   ...
   IF !so_um_reg.OR.(M->rcod1<=codigo.AND.M->rcod2>=codigo)
    ...
    IMPCTL(drvpcom)
		...
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,012 SAY GRUPOS->bairro+[ ]+GRUPOS->cobrador// Bairro
		@ cl,084 SAY GRUPOS->bairro// Bairro
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,012 SAY GRUPOS->cidade+[ ]+GRUPOS->uf// Bairro
		@ cl,084 SAY GRUPOS->cidade+[ ]+GRUPOS->uf// Bairro
		@ cl,052 SAY GRUPOS->telefone             // Telefone
		@ cl,122 SAY left(GRUPOS->telefone,14)
		REL_CAB(1)                                     // soma cl/imprime cabecalho
    ...
    ENDI
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,012 SAY LEFT(CLASSES->descricao,20) //+[ - ]+left(GRUPOS->relig,10),35) // Bairro
		@ cl,084 SAY CLASSES->descricao// Bairro
    IMPCTL(drvtcom)
    @ cl,002 say [ ]
    IF M->combarra=[S]
     CODBARRAS({{codigo+tipo+circ,4,13,22}},10,6)
    ENDI
		REL_CAB(1)                                    // soma cl/imprime cabecalho
    IF tipo#[1]
		 @ cl,010 SAY GRUPOS->codigo+[/]+tipo+circ //+[:]+STRZERO(VAL(circ)-VAL(GRUPOS->circinic)+1,3)
		 @ cl,052 SAY GRUPOS->codigo+[/]+tipo+circ+[:]+STRZERO(VAL(circ)-VAL(GRUPOS->circinic)+1,3)+[-]+left(GRUPOS->relig,7)
    ELSE
		 @ cl,010 SAY GRUPOS->codigo+[/]+tipo+circ
		 @ cl,052 SAY GRUPOS->codigo+[/]+tipo+circ+[-]+left(GRUPOS->relig,10) // Contrato
    ENDI
    IF M->combarra=[S]
     CODBARRAS({{codigo+tipo+circ,4,13,22}},10,6)
    ENDI
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,008 SAY DTOC(emissao_) // Vencimento 2
    ...
    IF M->combarra=[S]
     zeroval=RIGHT([00000]+ALLTRIM(STR(INT(valororig*100))),5)
     CODBARRAS({{DTOS(emissao_)+IF(EMPT(GRUPOS->COBRADOR),[000],GRUPOS->cobrador)+zeroval,4,20,08}},10,6)
    ENDI
		REL_CAB(1)                                     // soma cl/imprime cabecalho
...
// Processamentos
    ...
    IF so_um_reg .AND.(TAXAS->codlan=[EMC-])
     SELE EMCARNE
