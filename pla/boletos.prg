...
/*
 ...
 \ Programa: BOLETOS.PRG
 \ Data....: 01-02-98
 \ Sistema.: Administradora - PLANO
 \ Funcao..: Gerenciador do subsistema de boletos emitidos
 ...
 DO CASE
  CASE op_cad=01                                   // inclus„o
   ...
   IF AT("D",exrot[op_sis])=0                      // se usuario pode fazer inclusao
    BOL_INCL()                                     // neste arquivo chama prg de inclusao
...
PROC BOL_incl     // inclusao no arquivo BOLETOS
...
#ifdef COM_REDE
 BOL_CRIA_SEQ()                                    // cria dbf de controle de cp sequenciais
 FOR i=1 TO FCOU()                                 // cria/declara privadas as
...
BOL_TELA()                                         // imp tela para inclusao
 ...
 */
 ...
 cabem=INT((cabem-2048)/BOLETOS->(RECSIZE()))
 ...
 ELSE                                              // se nao rola a campos para cima
  ...
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+11,l_max-1,c_s+20,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+24,l_max-1,c_s+29,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+35,l_max-1,c_s+35,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+42,l_max-1,c_s+44,1)
 ENDI
ENDD

#ifdef COM_REDE
 BOL_ANT_SEQ()                                     // restaura sequencial anterior
 SELE BOLETOS
#endi
...
#ifdef COM_REDE
 PROC BOL_ANT_SEQ(est_seq)     // restaura sequencial anterior
 SELE BOL_SEQ     // seleciona arquivo de controle de sequencial
 BLOARQ(0,.5)     // esta estacao foi a ultima a incluir?
 IF sq_atual_ == seq
  REPL seq WITH sq_seq
  REPL nnumero WITH sq_nnumero
 ENDI
...
PROC BOL_CRIA_SEQ   // cria dbf de controle de campos sequenciais
LOCAL dbfseq_:=drvdbf+"BOL_seq" // arq temporario
...
IF RECC()=0                     // se o dbf foi criado agora
 ...
 IF RECC()>0                    // se o DBF nao estiver
  ...
  REPL BOL_SEQ->seq WITH seq
  REPL BOL_SEQ->nnumero WITH nnumero
  SET ORDER TO 1                // retorna ao indice principal
 ENDI
 SELE BOL_SEQ                   // seleciona arq de sequencias
 UNLOCK                         // libera DBF para outros usuarios
 COMMIT                         // atualiza cps sequenciais no disco
ENDI
RETURN
...
M->nnumero=LPAD(STR(VAL(nnumero)+1),10,[0])

#ifdef COM_REDE
 BOL_GRAVA_SEQ()
 sq_atual_=BOL_SEQ->seq
...
PROC BOL_GRAVA_SEQ
REPL seq WITH M->seq
REPL nnumero WITH M->nnumero
RETU

PROC BOL_tela     // tela do arquivo BOLETOS
tem_borda=.t.
...
SETCOLOR(drvcortel)
@ l_s+01,c_s+1 SAY " Seq.   ³ N.N£mero   ³ Contrato ³ Tipo ³ N.Circ"
@ l_s+02,c_s+1 SAY "ÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄ"
@ l_s+03,c_s+1 SAY "        ³            ³          ³      ³"
@ l_s+04,c_s+1 SAY "        ³            ³          ³      ³"
@ l_s+05,c_s+1 SAY "        ³            ³          ³      ³"
@ l_s+06,c_s+1 SAY "        ³            ³          ³      ³"
...
PROC BOL_gets     // mostra variaveis do arquivo BOLETOS
...
DO WHILE !EOF() .AND. l_s+l_a<l_max
 ...
 @ l_s+l_a,c_s+11 GET  nnumero;
                  PICT sistema[op_sis,O_CAMPO,02,O_MASC]

 @ l_s+l_a,c_s+24 GET  codigo;
                  PICT sistema[op_sis,O_CAMPO,03,O_MASC]

 @ l_s+l_a,c_s+35 GET  tipo;
                  PICT sistema[op_sis,O_CAMPO,04,O_MASC]

 @ l_s+l_a,c_s+42 GET  circ;
...
PROC BOL_get1     // capta variaveis do arquivo BOLETOS
LOCAL getlist := {}
PRIV  blk_boletos:=.t.
PARA tp_mov
IF tp_mov=INCLUI
