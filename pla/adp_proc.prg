...
@ 03,53 SAY drvdbf
SETCOLOR(drvtitbox)
CAIXA(mold,15,39,22,76)                // monta caixa da tela
SETCOLOR(drvcorbox)
@ 16,40 SAY "    PresServ Inform†tica Ltda. "
@ 17,40 SAY "       contato@presserv.com.br"
@ 18,40 SAY "(19)3452.3712 /skype:presservlimeira"
@ 19,40 SAY "        REPRODUCAO PROIBIDA"
@ 20,40 SAY "          ƒƒƒÕÕÕ * ÕÕÕƒƒƒ"
@ 21,40 SAY PADR(usuario,15)
@ 21,56 SAY PADR([ ],15)
@ 21,71 SAY LEFT(TIME(),5)
//POEHORA(21,71,5)
...
DO WHIL op_a>0
 ...
 op_a=DBOX(SUBS(msg,2),03,62,E_MENU,NAO_APAGA,"BASE DE DADOS",,,op_a)
 IF op_a>0                                      // escolheu um...
  ...
  FOR op_sis=ii TO nss+1                        // indexa "pais e filhos"
   ...
   FOR t=1 TO LEN(sistema[op_sis,O_INDIC])      // recria todos os ntx do arquivo
    ...
    INDE ON &chvind. TO (ntx) EVAL odometer() EVERY 50  // indexando...
   NEXT
   CLOS ALL
  NEXT
...
DO WHIL .t.
 ...
 cod_sos=42
 op_imp=DBOX(rel_imp,05,70,E_MENU,NAO_APAGA,"IMPRESSORAS|(DEL=APAGA)",,,op_imp)
...
op_6=1
m_criterio=""
DO WHIL selnum<10 .AND. cpsel!=SPAC(10)           // permite escolher somente 3 criterios
...
LOCAL op_sis, ind_ok, ind_an, arqtmp, chv_, p_ , x_, ind01, ind02, ind03, ind04, ind05
op_sis=EVAL(qualsis,ALIAS())                   // subscricao do vetor sistema
...
IF LEN(cpord)>1 .OR. !EMPT(criterio) .OR.;     // se tem ordenacao ou filtro
 ...
 IF !EMPT(criterio)                            // tem filtro?
  DBOX("Criando °ndice tempor†rio",5,,,NAO_APAGA)
  INDE ON &nord. TO (arqtmp) FOR &criterio EVAL odometer() EVERY 50// indexando...
 ELSE
  DBOX("Criando °ndice tempor†rio",5,,,NAO_APAGA)
  INDE ON &nord. TO (arqtmp) EVAL odometer() EVERY 50// indexando...
 ENDI

  RESTSCREEN(0,0,MAXROW(),79,ind_t)
  IF op_sis<=nss                       // se nao for as senhas,
   ind01=drvntx+sistema[op_sis,O_INDIC,1]
                                       //  coloca o path
  ELSE                                 // senao,
   ...
   ELSE
    ...
    ELSE
     ...
     ELSE
      ...
      IF qt_ind=5
       SET INDE TO (ind01), (ind02), (ind03), (ind04), (ind05), (arqtmp)
      ENDI
     ENDI
    ENDI
   ENDI
...
LOCAL co_r:=SETCOLOR(drvcorenf), MsgEsq, MsgCen, MsgDir
...
ELSE
 MsgDir=SPACE(9)
ENDI
@ l_i,c_s+1 SAY " "+MsgEsq+PADC(MsgCen,c_i-c_s-3-LEN(MsgEsq)-LEN(MsgDir))+MsgDir+" "
SETCOLOR(co_r)
RETU
...
PROC TIRA_LANC(db_,lan_)  // Retira o lancamento feito no arquivo alvo
