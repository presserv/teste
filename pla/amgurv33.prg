...
IF !so_um_reg                             // vai receber a variaveis?
 ...
 @ l_s+05,c_s+1 SAY " Valor p/atraso:  at‚:"
 @ l_s+06,c_s+1 SAY "                  at‚:"
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (tipo=[2].AND.(EMPT(M->rgrupo).OR.GRUPOS->grupo=M->rgrupo).AND.(M->rproxcirc=[000].OR.ARQGRUP->proxcirc=M->rproxcirc)) .OR. so_um_reg// se atender a condicao...
    ...
    SEEK chv071
    IF FOUND()
     DO WHIL ! EOF() .AND. chv071=LEFT(&(INDEXKEY(0)),LEN(chv071))
      #ifdef COM_TUTOR
       IF IN_KEY()=K_ESC                           // se quer cancelar
      #else
       IF INKEY()=K_ESC                            // se quer cancelar
...
IF !EMPT(M->valor1)                                // pode imprimir?
 @ 27,058 SAY [At‚:]+TRAN(M->ateh1,'@D')+[ ]+TRAN(M->valor1,'@E 999.99')// at‚1:
ENDI
IF !EMPT(M->valor2)                                // pode imprimir?
 @ 28,058 SAY [At‚:]+TRAN(M->ateh2,'@D')+[ ]+TRAN(M->valor2,'@E 999.99')// at‚2:
ENDI
...
IF cl>maxli .OR. qt=0                              // quebra de pagina
 ...
 IF !EMPT(M->valor1)                               // pode imprimir?
  @ 2,046 SAY [At‚:]+TRAN(M->ateh1,'@D')+[ ]+TRAN(M->valor1,'@E 999.99')// At‚1:
 ENDI
 @ 3,029 SAY TRAN(TAXAS->valor,"@E 999,999.99")    // Valor
 IF !EMPT(M->valor2)                               // pode imprimir?
  @ 3,046 SAY [At‚:]+TRAN(M->ateh2,'@D')+[ ]+TRAN(M->valor2,'@E 999.99')// At‚2:
 ENDI
 SELE (ar_)
 cl=qt+7 ; pg_++
ENDI
