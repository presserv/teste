...
/*
 ...
 \ Programa: ADM_R009.PRG
 \ Data....: 28-10-96
 \ Sistema.: Administradora - PLANO
 \ Funcao..: Grupos Prontos p/Emitir
...
BEGIN SEQUENCE
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  tot027010:=tot027011:=tot027012 := 0             // inicializa variaves de totais
  qqu027=0                                         // contador de registros
  DO WHIL !EOF()
   #ifdef COM_TUTOR
    ...
    ENDI
   ENDI
   IF (procpend>=acumproc.AND.(emissao_+periodic)<=M->remissao_).OR.(proxcirc>[000]).AND.!(CLASSES->prior=[S])// se atender a condicao...
    ...
    tot027011+=contrat
    @ cl,065 SAY TRAN(contrat,"999999")            // Contratos
    @ cl,071 SAY "/"
    tot027012+=partic
    @ cl,072 SAY TRAN(partic,"999999")             // Partic.
    qqu027++                                       // soma contadores de registros
    chv028=grup
    SELE PRCESSOS
    SEEK chv028
    IF FOUND()
     ...
     @ cl,000 SAY "========= ==== === ===== ==== ============================================"
     DO WHIL ! EOF() .AND. chv028=grup //LEFT(&(INDEXKEY(0)),LEN(chv028))
      #ifdef COM_TUTOR
       IF IN_KEY()=K_ESC                           // se quer cancelar
      #else
       IF INKEY()=K_ESC                            // se quer cancelar
      ...
      IF saiu<[001]                                // se atender a condicao...
       ...
       @ cl,015 SAY TRAN(grup,"!9")                // Grupo
       @ cl,019 SAY TRAN(num,"999999")             // Contrato
  ...
  ENDI                                             // salta para a proxima pagina
  @ ++cl,059 SAY REPL('-',19)
  @ ++cl,059 SAY TRAN(tot027010,"999")             // total Processos
  @ cl,065 SAY TRAN(tot027011,"999999")            // total Contratos
  @ cl,072 SAY TRAN(tot027012,"999999")            // total Partic.
  REL_CAB(2)                                       // soma cl/imprime cabecalho
  @ cl,000 SAY "*** Quantidade total "+TRAN(qqu027,"@E 999,999")
...
msgt="PROCESSAMENTOS DO RELAT¢RIO|GRUPOS PRONTOS P/EMITIR"
ALERTA()
...
IF cl>maxli .OR. qt=0                              // quebra de pagina
 ...
 @ 1,071 SAY "ADM_R009"                            // c¢digo relat¢rio
