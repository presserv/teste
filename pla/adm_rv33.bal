...
IF !so_um_reg                             // vai receber a variaveis?
 ...
 @ l_s+07,c_s+1 SAY " Cobradores:                           S/N:"
...
DO WHILE .t.
 ...
 IF !so_um_reg
  ...
  @ l_s+07 ,c_s+14 GET  rcob;
                   PICT "@S25"
                   AJUDA "Informe o cobrador a filtrar"

  @ l_s+07 ,c_s+45 GET  rcobsn;
                   PICT "!";
                   VALI CRIT("rcobsn$[SN ]~Digite S para imprimir|ou|N para nao imprimir")
                   AJUDA "Digite S para imprimir ou N para ignorar estes cobradores"
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.(TAXAS->codigo+TAXAS->tipo+TAXAS->circ=M->mcodaux))
   ...
   IF M->rcod2>[000000].AND.M->rcod2<codigo
    ...
    LOOP
   ENDI
   IF !EMPT(M->rcob)               // Se escolheu alguns cobradores
    IF GRUPOS->cobrador$M->rcob    // E este est  na lista...
     IF M->rcobsn=[N]              // E a lista for para NAO imprimir,
      SKIP                         // Entao cai fora...
      LOOP
     ENDI
    ELSE                           // Se nÆo est  na lista
     IF M->rcobsn=[S]              // E a lista por para imprimir
      SKIP                         // EntÆo cai fora...
      LOOP
     ENDI
    ENDI
