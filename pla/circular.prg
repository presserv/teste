...
PROC CIR_incl     // inclusao no arquivo CIRCULAR
...
PROC CIR_get1     // capta variaveis do arquivo CIRCULAR
LOCAL getlist := {}
PRIV  blk_circular:=.t.
PARA tp_mov
IF tp_mov=INCLUI
...
PROC CPR_incl     // inclusao no arquivo CPRCIRC
LOCAL getlist:={}, cabem:=1, rep:=ARRAY(FCOU()), ult_reg:=RECN(),;
 ...
 */
 ...
 ELSE                                              // se nao rola a campos para cima
  ...
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+63,l_max-1,c_s+70,1)
 ENDI
ENDD
GO ult_reg                                         // para o ultimo reg digitado
SETKEY(K_F3,t_f3_)                                 // restaura teclas de funcoes
...
PROC CPR_get1     // capta variaveis do arquivo CPRCIRC
LOCAL getlist := {}
PRIV  blk_cprcirc:=.t.
