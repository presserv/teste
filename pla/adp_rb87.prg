...
DO WHIL op_2=1 .AND. tps=1                         // teste de posicionamento
 ...
 ELSE                                              // testar posicionamento
  ...
  @ 002,049 SAY REPL("X",8)
  @ 006,000 SAY REPL("X",8)
  @ 006,009 SAY REPL("X",15)
  @ 006,035 SAY REPL("X",8)
  @ 008,049 SAY REPL("X",10)
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
    ...
    @ cl,049 SAY TRAN(emissao_,"@D")               // Emissao
    REL_CAB(4)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY TRAN(DATE(),"@D")                 // data1
    IMPCTL(drvpenf)
    @ cl,009 SAY GRUPOS->grupo+[ ]+GRUPOS->codigo+[ ]+tipo+[ ]+circ// Codigo
    IMPCTL(drvtenf)
    @ cl,035 SAY TRAN(DATE(),"@D")                 // data2
    REL_CAB(2)                                     // soma cl/imprime cabecalho
    IF r08701f9()                                  // pode imprimir?
    ENDI
     @ cl,049 SAY TRAN(R08702F9()+vlseg,"@E 999,999.99")// Valor Total
    REL_CAB(4)                                     // soma cl/imprime cabecalho
    ...
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY IIF(EMPT(M->rmens6),MENSAG->mens1,M->rmens6)// Mensagem 6
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY IIF(EMPT(M->rmens7),MENSAG->mens2,M->rmens7)// Mensagem 7
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY IIF(EMPT(M->rmens8),MENSAG->mens3,M->rmens8)// Mensagem 8
    REL_CAB(2)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY GRUPOS->nome                      // Nome
    @ cl,041 SAY TRAN(pg_,'9999')                  // n£mero da p gina
    REL_CAB(1)                                     // soma cl/imprime cabecalho
