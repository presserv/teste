...
PROC COB_incl     // inclusao no arquivo COBRADOR
...
PROC COB_get1     // capta variaveis do arquivo COBRADOR
LOCAL getlist := {}, t_f7_
PRIV  blk_cobrador:=.t.
PARA tp_mov
IF tp_mov=INCLUI
...
PROC COB_MEMO
IF READVAR()="MEMO09"
 EDIMEMO("obs",sistema[op_sis,O_CAMPO,09,O_TITU],14,2,23,37)
ENDI
RETU

PROC PCB_incl     // inclusao no arquivo PCBRAD
LOCAL getlist:={}, cabem:=1, rep:=ARRAY(FCOU()), ult_reg:=RECN(),;
...
PCB_TELA()                                         // imp tela para inclusao
INFOSIS()                                          // exibe informacao no rodape' da tela
 ...
 */
 ...
 NEXT
 cabem=DISKSPACE(IF(LEN(drvdbf)<2.OR.drvdbf="\",0,ASC(drvdbf)-64))
 cabem=INT((cabem-2048)/PCBRAD->(RECSIZE()))
 IF cabem<1                                        // mais nenhum!!!
  ALERTA()
  msg="Verifique ESPA€O EM DISCO, "+usuario
  DBOX(msg,,,,,"INCLUSŽO INTERROMPIDA!")           // vamos parar por aqui!
 ...
 */
 ...
 IF LASTKEY()=K_ESC                                // cancelou ou chave em branco
  ...
  LOOP                                             // volta p/ menu de cadastramento
 ENDI
 SEEK M->cobrador+M->tipo+M->circ
 aprov_reg_=(FOUND().AND.DELE().AND.!drvvisivel)   // vai aproveitar o registro?
 IF FOUND() .AND. !aprov_reg_                      // pesquisou e achou!
  l_a=Sistema[op_sis,O_TELA,O_SCROLL]
  op_menu=ALTERACAO                                // seta flag de ateracao
  DISPBEGIN()
  PCB_GETS()                                       // mostra conteudo do registro
  INFOSIS()                                        // exibe informacao no rodape' da tela
  DISPEND()
  ALERTA()
  msg="Consultar/alterar|Retornar … inclus„o"      // pergunta se deseja
 ...
 ENDI
  ...
  SKIP                                             // arq para que nehum outro
  BLOREG(0,.5)                                     // usuario possa incluir
  SEEK M->cobrador+M->tipo+M->circ                 // se registro foi incluido
  aprov_reg_=(FOUND().AND.DELE().AND.!drvvisivel)  // vai aproveitar o registro?
  IF FOUND() .AND. !aprov_reg_                     // por outro usuario, entao
   BLOREG(0,.5)
   FOR i=1 TO FCOU()
    ...
    REPL &msg. WITH M->&msg.
   NEXT
   PCB_GET1(FORM_INVERSA)                          // executa formula inversa
   RECA
   FOR i=1 TO FCOU()
    msg=FIEL(i)
    REPL &msg. WITH rep[i]
 ...
 ELSE                                              // se nao rola a campos para cima
  ...
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+22,l_max-1,c_s+22,1)
  SCROLL(l_s+Sistema[op_sis,O_TELA,O_SCROLL],c_s+33,l_max-1,c_s+38,1)
...
PROC PCB_tela     // tela do arquivo PCBRAD
tem_borda=.t.
l_s=Sistema[op_sis,O_TELA,O_LS]           // coordenadas da tela
c_s=Sistema[op_sis,O_TELA,O_CS]
l_i=Sistema[op_sis,O_TELA,O_LI]
...
@ l_s,c_s-1+(c_i-c_s+1)/2-i SAY " "+MAIUSC(sistema[op_sis,O_MENS])+" "
SETCOLOR(drvcortel)
@ l_s+01,c_s+1 SAY " Tipo ³ Circ ³Incluir Seguro?³ Comiss„o% Valor"
@ l_s+02,c_s+1 SAY "ÄÄÄÄÄÄÅÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ"
@ l_s+03,c_s+1 SAY "      ³      ³               ³"
@ l_s+04,c_s+1 SAY "      ³      ³               ³"
@ l_s+05,c_s+1 SAY "      ³      ³               ³"
@ l_s+06,c_s+1 SAY "      ³      ³               ³"
...
PROC PCB_gets     // mostra variaveis do arquivo PCBRAD
...
DO WHILE !EOF() .AND. l_s+l_a<l_max .AND.;
 ...
 @ l_s+l_a,c_s+22 GET  seg;
                  PICT sistema[op_sis,O_CAMPO,04,O_MASC]

 @ l_s+l_a,c_s+33 GET  pcomiss;
                  PICT sistema[op_sis,O_CAMPO,05,O_MASC]

 @ l_s+l_a,c_s+42 GET  vlcomiss;
...
PROC PCB_get1     // capta variaveis do arquivo PCBRAD
LOCAL getlist := {}
PRIV  blk_pcbrad:=.t.
...
ELSEIF tp_mov=INCLUI .OR. tp_mov=RECUPERA .OR. tp_mov=FORM_DIRETA
 IF (op_menu=INCLUSAO .AND. LASTKEY()!=K_ESC) .OR. op_menu!=INCLUSAO
  IF tp_mov=RECUPERA .AND. op_menu!=INCLUSAO .AND. (COBRADOR->(DELE()))
   msg="|"+sistema[EVAL(qualsis,"COBRADOR"),O_MENU]
   ALERTA(2)
...
PROC FCC_incl     // inclusao no arquivo FCCOB
LOCAL getlist:={}, cabem:=1, rep:=ARRAY(FCOU()), ult_reg:=RECN(),;
      ctl_r, ctl_c, t_f3_, t_f4_, l_max, dele_atu:=SET(_SET_DELETED,.f.)
PRIV op_menu:=INCLUSAO, op_sis, l_s, c_s, l_i, c_i, cod_sos, tem_borda, criterio:="", cpord:="", l_a
op_sis=EVAL(qualsis,"FCCOB")
IF AT("D",exrot[op_sis])>0
 RETU
ENDI
FOR i=1 TO FCOU()                                  // cria/declara privadas as
...
DISPBEGIN()                                        // monta tela na pagina de traz
IMPRELA()                                          // imp telas do pai
FCC_TELA()                                         // imp tela para inclusao
INFOSIS()                                          // exibe informacao no rodape' da tela
l_a=Sistema[op_sis,O_TELA,O_SCROLL]
DISPEND()                                          // apresenta tela de uma vez so
DO WHIL cabem>0
 ...
 */
 ...
 NEXT
 cabem=DISKSPACE(IF(LEN(drvdbf)<2.OR.drvdbf="\",0,ASC(drvdbf)-64))
 cabem=INT((cabem-2048)/FCCOB->(RECSIZE()))
 IF cabem<1                                        // mais nenhum!!!
  ALERTA()
  msg="Verifique ESPA€O EM DISCO, "+usuario
  DBOX(msg,,,,,"INCLUSŽO INTERROMPIDA!")           // vamos parar por aqui!
 ...
 */
 ...
 IF LASTKEY()=K_ESC                                // cancelou ou chave em branco
  ...
  LOOP                                             // volta p/ menu de cadastramento
 ENDI
 SEEK M->cobrador+M->mesref
 aprov_reg_=(FOUND().AND.DELE().AND.!drvvisivel)   // vai aproveitar o registro?
 IF FOUND() .AND. !aprov_reg_                      // pesquisou e achou!
  l_a=Sistema[op_sis,O_TELA,O_SCROLL]
  op_menu=ALTERACAO                                // seta flag de ateracao
  DISPBEGIN()
  FCC_GETS()                                       // mostra conteudo do registro
  INFOSIS()                                        // exibe informacao no rodape' da tela
  DISPEND()
  ALERTA()
  msg="Consultar/alterar|Retornar … inclus„o"      // pergunta se deseja
 ...
 ENDI
  ...
  SKIP                                             // arq para que nehum outro
  BLOREG(0,.5)                                     // usuario possa incluir
  SEEK M->cobrador+M->mesref                       // se registro foi incluido
  aprov_reg_=(FOUND().AND.DELE().AND.!drvvisivel)  // vai aproveitar o registro?
  IF FOUND() .AND. !aprov_reg_                     // por outro usuario, entao
   BLOREG(0,.5)
   FOR i=1 TO FCOU()
    ...
    REPL &msg. WITH M->&msg.
   NEXT
   FCC_GET1(FORM_INVERSA)                          // executa formula inversa
   RECA
   FOR i=1 TO FCOU()
    msg=FIEL(i)
    REPL &msg. WITH rep[i]
...
PROC FCC_tela     // tela do arquivo FCCOB
tem_borda=.t.
l_s=Sistema[op_sis,O_TELA,O_LS]           // coordenadas da tela
c_s=Sistema[op_sis,O_TELA,O_CS]
l_i=Sistema[op_sis,O_TELA,O_LI]
...
PROC FCC_gets     // mostra variaveis do arquivo FCCOB
LOCAL getlist := {}, l_max, reg_atual:=RECNO()
PRIV  l_a:=Sistema[op_sis,O_TELA,O_SCROLL]
FCC_TELA()
l_max=l_s+Sistema[op_sis,O_TELA,O_SCROLL]+Sistema[op_sis,O_TELA,O_QTDE]
...
PROC FCC_get1     // capta variaveis do arquivo FCCOB
LOCAL getlist := {}
PRIV  blk_fccob:=.t.
PARA tp_mov, excl_rela
excl_rela=IF(excl_rela=NIL,.f.,excl_rela)
