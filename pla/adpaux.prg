...
#ifdef COM_LOCK
 pr_ok=__PPRJ(arq_sos,"ùÿäòäåúúçóàêþþæñëâþþ")
 IF LEN(pr_ok)>0
  CLEAR
  ? pr_ok
  RETU
...
datac=DBOX(msg,,,,,"ATEN€ŽO!",datac,"99/99/9999")    // confirma a data
...
*/
   ...
   CASE opc_01=2     // vendas
    ...
    DO WHIL opc_02!=0
     cod_sos=4
     RESTSCREEN(0,0,MAXROW(),79,v02)
     menu02="Gerar carnˆ do contrato|"+;
            "Lan‡amento/Carnˆs|"+;
	    "Emiss„o Carnˆ|"+;
	    "Etiqueta p/Carnˆs|"+;
			"Impress„o Contrato|"+;
     ...
     ROLAPOP()
     DO CASE
      CASE opc_02=1     // gerar carnˆ do contrato
       ADP_P007(3,23)

      CASE opc_02=2     // lan‡amento/carnˆs
       EMCARNE(3,23)


      CASE opc_02=3     // emiss„o carnˆ
       IF p_recp==[S]
        ADM_RP38(3,23)
       ELSE
        ADP_R066(3,23)
      ...
      CASE opc_02=4     // etiqueta p/carnˆs
       ADM_E002(3,23)
      ...
      CASE opc_02=7     // relat¢rios
       ...
       v03=SAVESCREEN(0,0,MAXROW(),79)
        ...
        DO CASE
         CASE opc_03=1     // contratos sem carnˆs
          ADP_RX85(5,31)
     ...
     menu02="Relatorios anteriores|"+;
      ...
      CASE opc_02=2     // gera‡„o dos d‚bitos
       ...
       DO WHIL opc_03!=0
				cod_sos=4
        RESTSCREEN(0,0,MAXROW(),79,v03)
        menu03="D‚bitos do mˆs|"+;
               "D‚bitos do Per¡odo"
        msgt="GERA€ŽO DOS DBITOS"
        opc_03=DBOX(menu03,3,31,E_MENU,NAO_APAGA,msgt,,,opc_03)
        DO CASE
         CASE opc_03=1     // d‚bitos do mˆs
          ADP_P001(5,39)
...
//        CLOS ALL
      ...
      CASE opc_02=10     // proc.transferˆncias
       ADP_P003(3,31)
   ...
   CASE opc_01=5     // tabelas
    ...
    DO WHIL opc_02!=0
     ...
     DO CASE
      ...
      CASE opc_02=7     // parƒmetro de juros
       JUROS(3,53)
