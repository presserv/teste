...
IF !EMPTY(drvtapg)                                 // existe configuracao de tam pag?
 ...
 lpp_017=LEFT(drvtapg,op_-1)+"017"+SUBS(drvtapg,op_+3)
 lpp_066=LEFT(drvtapg,op_-1)+"066"+SUBS(drvtapg,op_+3)
ELSE
 lpp_016:=lpp_017:=lpp_066 :=""                             // nao ira mudara o tamanho da pag
ENDI
DBOX("[ESC] Interrompe",15,,,NAO_APAGA)
dele_atu:=SET(_SET_DELETED,.t.)                    // os excluidos nao servem...
SET DEVI TO PRIN                                   // inicia a impressao
maxli=15                                           // maximo de linhas no relatorio
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
    ...
    IMPAC("+------ Contrato -------------+--  Emiss„o  --------+ : +------ Contrato -------------+--  Emiss„o  --------+",cl,000)
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    IMPEXP(cl,003,GRUPOS->grupo+'  '+GRUPOS->codigo,20)// Codigo 2
    @ cl,030 SAY "| Manut.:"
    valororig=R08702F9()                           // variavel temporaria
    @ cl,040 SAY TRAN(valororig,"@E 999,999.99")   // Valor 1
    @ cl,052 SAY "| : |"
    IMPEXP(cl,060,GRUPOS->grupo+'  '+GRUPOS->codigo,20)// Codigo 3
    @ cl,086 SAY "| Manut.:"
    @ cl,096 SAY TRAN(M->valororig,"@E 999,999.99")// Valor 2
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+-----------------------------+ Seguro:"
    ...
    @ cl,030 SAY "| Total.:"
    @ cl,040 SAY TRAN(valororig+vlseg,"@E 999,999.99")// Valor Total
    @ cl,052 SAY "| : |"
    @ cl,058 SAY TAXAS->tipo+[  ]+TAXAS->circ+IIF(tipo$[16].AND.EMCARNE->(EOF()),[],[/]+STRZERO(TCARNES->parf,3))// N§ 2
    IMPEXP(cl,069,TRAN(TAXAS->emissao_,"@D"),16)   // Vencto 2
    @ cl,086 SAY "| Total.:"
    @ cl,096 SAY TRAN(valororig+vlseg,"@E 999,999.99")// Valor Total
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+-----------------------------+---------------------+ : +-----------------------------+---------------------+"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    ...
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+---------------------------- 1¦Via - Contribuinte -+ : +--------------------------------- 2¦Via - Empresa -+"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,001 SAY "úúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúú:úúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúúú"
    cl:=999
...
IF cl>maxli .OR. qt=0                              // quebra de pagina
 ...
 ELSE
  maxli=16
  IMPCTL(lpp_017)                                    // seta pagina com 16 linhas
 ENDI
 @ 0,000 SAY "+---------------------------------------------------+ : +---------------------------------------------------+"
 @ 1,000 SAY "|"
