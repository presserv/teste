...
DO WHILE .t.
 ...
 IF !so_um_reg
  ...
  @ l_s+21 ,c_s+30 GET  confirme;
                   PICT "!";
                   VALI CRIT("confirme$'SB23'.AND.V87001F9()~CONFIRME n„o aceit vel")
...
/*
   ...
   ENDI
*/
   IF !EMPT(M->rcod1).and.confirme$[S23]
...
*/
   ...
   ENDI
    ...
    rcbarra:=TAXAS->codigo+TAXAS->tipo+TAXAS->circ      // Nosso n£mero
    ...
    rcbarra:=DG_COBSI(TAXAS->codigo+TAXAS->tipo+TAXAS->circ)// DV p/Nosso N£mero
...
ENDI
    ...
    FOR contx:=1 TO 10                             // Falecidos
     ...
     IF contx=2
      IMPCTL(drvtcomx)
      @ cl+2,058 SAY STR(TAXAS->valor,7,2)
      IMPCTL(drvpcomx)
     ENDI
    NEXT contx
...
STAT FUNC MONTA_BARRA(wparcodemp,wnumcef,wti1valtit)
   ...
   ct1cabpart:= "000" + "0" + SubStr(strzero(wti2numcef, 11), 1, 5)
