...
maxli=50                                           // maximo de linhas no relatorio
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
    ...
    ENDI
    @ cl,024 SAY M->setup2                         // Ident2
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,024 SAY M->setup3                         // Setup3
    REL_CAB(3)                                     // soma cl/imprime cabecalho
    @ cl,004 SAY TRAN(GRUPOS->grupo,"!!")          // Grupo 1
    @ cl,014 SAY TRAN(codigo,"999999")             // Contrato
    @ cl,026 SAY TRAN(tipo,"!")                    // Tipo
    @ cl,027 SAY TRAN(circ,"999")                  // Circular
    @ cl,036 SAY TRAN(GRUPOS->admissao,"@D")       // Admiss„o
    REL_CAB(3)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY TRAN(emissao_,"@D")               // Emissao
    valororig=R08702F9()                           // variavel temporaria
    @ cl,013 SAY TRAN(valororig,"@E 999,999.99")   // Valor Taxa
    @ cl,027 SAY TRAN(GRUPOS->qtcircpg,"999")      // Circ.Pagas
		@ cl,037 SAY TRAN(cobrador,"!!!")               // Cobrador
    REL_CAB(2)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY GRUPOS->nome                      // Nome
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY GRUPOS->endereco                  // Endere‡o
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY GRUPOS->bairro                    // Bairro
    @ cl,025 SAY GRUPOS->cidade                    // Cidade
    @ cl,051 SAY TRAN(GRUPOS->uf,"!!")             // UF
    REL_CAB(16)                                    // soma cl/imprime cabecalho
    @ cl,002 SAY CIRCULAR->menscirc
    REL_CAB(01)                                    // soma cl/imprime cabecalho
    @ cl,002 SAY CIRCULAR->menscirc1
    REL_CAB(01)                                    // soma cl/imprime cabecalho
    @ cl,002 SAY CIRCULAR->menscirc2
    REL_CAB(03)                                    // soma cl/imprime cabecalho
    @ cl,011 SAY [Tiete           Vencimento]      // Tiete
    @ cl,051 SAY [Tiete           Vencimento]      // tiete 2
    REL_CAB(3)                                     // soma cl/imprime cabecalho
    @ cl,029 SAY TRAN(TAXAS->emissao_,"@D")        // Vencto 1
    @ cl,068 SAY TRAN(TAXAS->emissao_,"@D")        // Vencto 2
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY &drvpcom+GRUPOS->nome+&drvtcom    // Nome 2
    @ cl,058 SAY &drvpcom+GRUPOS->nome+&drvtcom    // Nome 3
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY &drvpcom+GRUPOS->endereco+&drvtcom// Endere‡o 2
    @ cl,058 SAY &drvpcom+GRUPOS->endereco+&drvtcom// Endere‡o 3
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY &drvpcom+GRUPOS->bairro+ALLTRIM(GRUPOS->cidade)+[, ]+GRUPOS->uf+&drvtcom// Bairro 2
    @ cl,058 SAY &drvpcom+GRUPOS->bairro+ALLTRIM(GRUPOS->cidade)+[, ]+GRUPOS->uf+&drvtcom// Bairro 3
    REL_CAB(3)                                     // soma cl/imprime cabecalho
    @ cl,002 SAY GRUPOS->grupo+[ ]+tipo+circ+[]+codigo+[ ]+ALLTRIM(STR(valor,9,2))// Grupo 2
    @ cl,042 SAY GRUPOS->grupo+[ ]+tipo+circ+[]+codigo+[ ]+ALLTRIM(STR(valor,9,2))// Grupo 3
    IF M->combarra=[S]
...
msgt="PROCESSAMENTOS DO RELAT¢RIO|RECIBO TIETE"
ALERTA()
