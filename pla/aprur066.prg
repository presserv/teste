...
/*
 ...
 \ Programa: APRUR066.PRG
 \ Data....: 08-11-97
 \ Sistema.: Administradora - PLANO
 \ Funcao..: Emiss„o Carnˆ - Petr¢polis
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
    ...
    lin_aux[1]+=PADR(LEFT(M->setup1,38),39)                // Ident1
    lin_aux[2]+=PADR(TRAN(M->cgcsetup,"@R 99.999.999/9999-99")+[  PMP 63113],39)// CGC
    lin_aux[4]+=PADR(LEFT([NOME: ]+GRUPOS->nome,38),39)    // NOME
    lin_aux[5]+=PADR([CONTRATO N§ ]+GRUPOS->codigo+[     GRUPO  ]+GRUPOS->grupo,39)// Codigo 2
    lin_aux[6]+=PADR([PRESTACAO N§ ]+circ+[ ]+TRAN(TCARNES->parf,[99]),39)// compl.codigo 1
    valororig=[VALOR AT  ]+DTOC(emissao_)+[    R$:]+ALLTRIM(TRAN(R08702F9(),[@E 999,999.99]))// variavel temporaria
    lin_aux[7]+=PADR(valororig,39)			 // Valor
    lin_aux[8]+=PADR("APOS O VENCIMENTO, JUROS DE 2% AO MES",39)
    lin_aux[9]+=PADR("ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿",39)
    lin_aux[10]+=PADR("³                                     ³",39)
    lin_aux[11]+=PADR("ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ",39)
    linax:=lin_aux[1]
    if len(linax)>50
     REL_CAB(1)                                     // soma cl/imprime cabecalho
     IMPCTL(drvpenf)
     @ cl,001 SAY lin_aux[1]                // Ident1
     IMPCTL(drvtenf)
     REL_CAB(1)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[2]
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[4]    // NOME
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[5] // Codigo 2
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[6]
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[7]			 // Valor
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     @ cl,001 SAY lin_aux[8]
     REL_CAB(1)                                     // soma cl/imprime cabecalho
     @ cl,000 SAY lin_aux[9]
     REL_CAB(1)                                     // soma cl/imprime cabecalho
     @ cl,000 SAY lin_aux[10]
     REL_CAB(1)                                     // soma cl/imprime cabecalho
     @ cl,000 SAY lin_aux[11]
     REL_CAB(2)                                     // soma cl/imprime cabecalho
     IMPCTL(drvpenf)
     IMPCTL(drvpenf)
     @ cl,001 SAY lin_aux[1]                // Ident1
...
msgt="PROCESSAMENTOS DO RELAT¢RIO|EMISSŽO CARNE"
ALERTA()
 ...
 DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
  IF (R08701F9()) .OR. so_um_reg                   // se atender a condicao...
    ...
    IF stat < [2]
     REPBLO('TAXAS->stat',{||[2]})
    ENDI
    IF M->rtp=[1].AND.VAL(circ)=TCARNES->parf.AND.EMPT(EMCARNE->emissao_)
