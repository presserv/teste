...
/*
 ...
 \ Programa: APCAR066.PRG
 \ Data....: 25-02-98
 \ Sistema.: Administradora - PLANO
 \ Funcao..: Carnˆ Capivari
...
IF FILE('PR066VAR.MEM')
 REST FROM PR066VAR ADDITIVE
ENDI
DO WHILE .t.
 rola_t=.f.
 cod_sos=56
...
IF !EMPTY(drvtapg)                                 // existe configuracao de tam pag?
 ...
 lpp_016=LEFT(drvtapg,op_-1)+"033"+SUBS(drvtapg,op_+3)
 lpp_066=LEFT(drvtapg,op_-1)+"066"+SUBS(drvtapg,op_+3)
ELSE
 lpp_016:=lpp_066 :=""                             // nao ira mudara o tamanho da pag
ENDI
SAVE TO PR066VAR ALL LIKE R*
  ...
  DO WHIL !EOF().AND.(!so_um_reg.OR.imp_reg=RECN())
   ...
   IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
    ...
    IF R08701F9()                                     // pode imprimir?
     @ cl,005 SAY M->setup1                            // Ident1
    ENDI
    @ cl,052 SAY "| : |"
    @ cl,063 SAY M->setup1                             // Ident1
    @ cl,108 SAY "|"
    REL_CAB(1)
    @ cl,000 SAY "|"
    @ cl,001 SAY M->setup2                             // Ident2
    @ cl,052 SAY "| : |"
    @ cl,057 SAY M->setup2                             // Ident2
    @ cl,108 SAY "|"
    REL_CAB(1)
    @ cl,000 SAY "|"
    @ cl,001 SAY M->setup3                             // Setup3
    @ cl,052 SAY "| : |"
    @ cl,057 SAY M->setup3                             // Setup3
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    IMPAC("+------ Contrato -------------+--  Emissao  --------+ : +------ Contrato -------------+--  Emissao  --------+",cl,000)
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    IMPEXP(cl,003,GRUPOS->grupo+'  '+GRUPOS->codigo,20)// Codigo 2
    @ cl,027 SAY "| Manut.:"
    valororig=R08702F9()                           // variavel temporaria
    @ cl,037 SAY TRAN(valororig,"@E 999,999.99")   // Valor 1
    @ cl,049 SAY "| : |"
    IMPEXP(cl,057,GRUPOS->grupo+'  '+GRUPOS->codigo,20)// Codigo 3
    @ cl,080 SAY "| Manut.:"
    @ cl,090 SAY TRAN(M->valororig,"@E 999,999.99")// Valor 2
    @ cl,102 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+-----------------------------+ Seguro:"
    @ cl,040 SAY TRAN(vlseg,"@E 999,999.99")       // Valor Seg
    @ cl,052 SAY "| : +-----------------------------+ Seguro:"
    @ cl,096 SAY TRAN(vlseg,"@E 999,999.99")       // Valor Seg.
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    @ cl,002 SAY TAXAS->tipo+[  ]+TAXAS->circ+IIF(tipo$[16].AND.EMCARNE->(EOF()),[],[/]+STRZERO(TCARNES->parf,3))// N§ 1
    IMPEXP(cl,013,TRAN(TAXAS->emissao_,"@D"),16)   // Vencto 1
    @ cl,027 SAY "| Total.:"
    @ cl,037 SAY TRAN(valororig+vlseg,"@E 999,999.99")// Valor Total
    @ cl,049 SAY "| : |"
    @ cl,055 SAY TAXAS->tipo+[  ]+TAXAS->circ+IIF(tipo$[16].AND.EMCARNE->(EOF()),[],[/]+STRZERO(TCARNES->parf,3))// N§ 2
    IMPEXP(cl,066,TRAN(TAXAS->emissao_,"@D"),16)   // Vencto 2
    @ cl,080 SAY "| Total.:"
    @ cl,090 SAY TRAN(valororig+vlseg,"@E 999,999.99")// Valor Total
    @ cl,102 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+-----------------------------+---------------------+ : +-----------------------------+---------------------+"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    ...
    @ cl,052 SAY "| : |Titular.:"
    @ cl,069 SAY GRUPOS->nome                      // Nome
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    IMPAC("|Endereco:",cl,000)
    @ cl,012 SAY GRUPOS->endereco                  // Endere‡o
    IMPAC("| : |Endereco:",cl,052)
    @ cl,069 SAY GRUPOS->endereco                  // Endere‡o
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "|"
    ...
    @ cl,052 SAY "| : |"
    @ cl,058 SAY GRUPOS->bairro                    // Bairro
    @ cl,079 SAY GRUPOS->cidade                    // Cidade
    @ cl,105 SAY TRAN(GRUPOS->uf,"!!")             // UF 2
    @ cl,108 SAY "|"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,000 SAY "+---------------------------- 1.Via - Contribuinte -+ : +-"
    @ cl,058 SAY TRAN(pg_,'9999')                  // n£mero da p gina
    @ cl,062 SAY "---------------------------- 2.Via - Empresa -+"
    REL_CAB(1)                                     // soma cl/imprime cabecalho
    @ cl,001 SAY ". . . . . . . . . . . . . . . . . . . . . . . . . . .:. . . . . . . . . . . . . . . . . . . . . . . . . . . "
    cl+=2
...
msgt="PROCESSAMENTOS DO RELAT¢RIO|CARNE CAPIVARI"
ALERTA()
...
/*
 ...
 IF R08701F9()                                     // pode imprimir?
  @ 1,005 SAY M->setup1                            // Ident1
 ENDI
 @ 1,052 SAY "| : |"
 @ 1,063 SAY M->setup1                             // Ident1
 @ 1,108 SAY "|"
 @ 2,000 SAY "|"
 @ 2,001 SAY M->setup2                             // Ident2
 @ 2,052 SAY "| : |"
 @ 2,057 SAY M->setup2                             // Ident2
 @ 2,108 SAY "|"
 @ 3,000 SAY "|"
 @ 3,001 SAY M->setup3                             // Setup3
 @ 3,052 SAY "| : |"
 @ 3,057 SAY M->setup3                             // Setup3
 @ 3,108 SAY "|"
 cl=qt+3 ; pg_++
 */
 cl=qt ; pg_++
