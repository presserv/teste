...
METHOD Add( oBol ) CLASS oRemessa
   ...
   IF ::CNAB400
      ...
      CASE ::cCodBco $ "237" // Bradesco
          ...
          ::cLine += PADL(oBol:cNumDoc, 25)
          ::cLine += "237" // banco debito automatico
          ::cLine += "20300" //zeros
		
		  cDGNN := DC_Mod11(::cCodBco, 7, .F., oBol:cCarteira + oBol:cNossoNumero)
          IF val(oBol:cNossoNumero) # 0
		  // padl(oBol:cNumAgencia ,4 ,"0") +;
              ::cLine +=PADL(oBol:cNossoNumero,11) +cDGNN
          ELSE
          ...
          ::cLine += PAD(RetiraAcento(oBol:Endereco), 40)
          ::cLine += SPACE(12)
          ::cLine += PAD(oBol:CEP, 8)

          cMsg := STRTRAN(PAD(oBol:cidade, 15)+PAD(oBol:Estado, 2), CRLF, " ")
...
METHOD Line() CLASS oRemessa
   ...
   IF ::CNAB400
      ...
      IF LEN(::cLine) != 400 // Erro!
         ...
         alert(ErrorNew("oRemessa", 0, 400, ProcName(), "Arquivos CNAB400 possuem 400 bytes por registro" + " (" + LTRIM(STR(LEN(::cLine))) + ")", HB_aParams()))
      ENDIF
   ELSE
      FWRITE(::nHandle, ::cLine + CRLF)
      IF LEN(::cLine) != 240 // Erro!
         FWRITE(::nHandle, STRZERO(LEN(::cLine), 5) + CRLF)
         alert(ErrorNew("oRemessa", 0, 240, ProcName(), "Arquivos FEBRABAN possuem 240 bytes por registro" + " (" + LTRIM(STR(LEN(::cLine))) + ")", HB_aParams()))
      ENDIF
   ENDIF
   ::cLine := ""
