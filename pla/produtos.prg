...
/*
 ...
 \ Funcao..: Gerenciador do subsistema de produtos
...
PROC PRO_incl     // inclusao no arquivo PRODUTOS
 ...
 */
 ...
 IF FOUND() .AND. !aprov_reg_                      // pesquisou e achou!
  ...
  DISPBEGIN()
  PRO_GETS()                                       // mostra conteudo do registro
  INFOSIS()                                        // exibe informacao no rodape' da tela
  DISPEND()
  ALERTA()
 ...
 ENDI
  ...
  IF FOUND() .AND. !aprov_reg_                     // por outro usuario, entao
   ...
   FOR i=1 TO FCOU()
    ...
    REPL &msg. WITH M->&msg.
   NEXT
   PRO_GET1(FORM_INVERSA)                          // executa formula inversa
   RECA
   FOR i=1 TO FCOU()
    msg=FIEL(i)
    REPL &msg. WITH rep[i]
...
PROC PRO_get1     // capta variaveis do arquivo PRODUTOS
LOCAL getlist := {}, t_f7_
PRIV  blk_produtos:=.t.
PARA tp_mov
IF tp_mov=INCLUI
