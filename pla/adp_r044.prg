...
//   IF /* !so_um_reg.OR. */ (M->rcod1<=codigo.AND.M->rcod2>=codigo)
   IF (codigo>=M->rcod1.AND.codigo<=M->rcod2)
	 ...
	 IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
		...
		ENDI
		 ...
		 @ cl,004 SAY LEFT(GRUPOS->nome,35)                       // Nome
		 @ cl,043 SAY LEFT(GRUPOS->nome,35)                       // Nome
		 REL_CAB(1)                                      // soma cl/imprime cabecalho
		 @ cl,004 SAY GRUPOS->endereco                   // Endere‡o
		 @ cl,043 SAY LEFT(GRUPOS->endereco,35)                   // Endere‡o
		 REL_CAB(1)                                      // soma cl/imprime cabecalho
		 @ cl,004 SAY LEFT(ALLTRIM(GRUPOS->bairro)+[,]+ALLTRIM(GRUPOS->cidade),35)      // Endere‡o
		 @ cl,043 SAY LEFT(ALLTRIM(GRUPOS->bairro)+[,]+ALLTRIM(GRUPOS->cidade),35)      // Endere‡o
		 ...
		 //ELSE
			@ cl,011 SAY TAXAS->tipo+[-]+TAXAS->circ
		 //ENDI
		 @ cl,017 SAY left(dtoc(TAXAS->emissao_),6)+right(dtoc(TAXAS->emissao_),2)
		 @ cl,024 SAY TRAN(valororig,"@E 999,999.99")
