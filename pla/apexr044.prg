...
IF !so_um_reg                             // vai receber a variaveis?
 ...
 @ l_s,c_s+14 SAY " IMPRESSŽO DE COBRAN€A "
 SETCOLOR(drvcortel)
...
maxli=16                                           // maximo de linhas no relatorio
 ...
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
   ...
   IF !so_um_reg.OR.(M->rcod1<=codigo.AND.M->rcod2>=codigo)
	 ...
	 IF (R08701F9()) .OR. so_um_reg                  // se atender a condicao...
		...
		@ cl,002 SAY left(GRUPOS->nome,35)             // Nome
		@ cl,045 SAY left(GRUPOS->nome,31)						// Nome 2
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,004 SAY left(GRUPOS->endereco,35)									// Endere‡o
		@ cl,046 SAY left(GRUPOS->endereco,30)									// Endere‡o
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,002 SAY &drvPcom+GRUPOS->bairro+SPACE(11)+GRUPOS->cidade+[ ]+GRUPOS->uf+&drvtcom// Bairro
		@ cl,044 SAY &drvPcom+GRUPOS->bairro+SPACE(09)+GRUPOS->cidade+[   ]+GRUPOS->uf+&drvtcom// Bairro
		REL_CAB(3)                                     // soma cl/imprime cabecalho
		@ cl,000 SAY GRUPOS->grupo                     // Grupo
		IMPCTL(drvpenf)
		@ cl,006 SAY GRUPOS->codigo                    // Contrato
		IMPCTL(drvtenf)
		@ cl,016 SAY LEFT(DTOC(DATE()),6)+RIGHT(DTOC(DATE()),2)// emissao
		@ cl,026 SAY TRAN(vlseg,"@E 999,999.99")       // Valor Seguro
		@ cl,041 SAY GRUPOS->grupo                     // Grupo 2
		IMPCTL(drvpenf)
		@ cl,047 SAY GRUPOS->codigo                    // Contrato 2
		IMPCTL(drvtenf)
		@ cl,057 SAY LEFT(DTOC(DATE()),6)+RIGHT(DTOC(DATE()),2)// emissao2
		@ cl,069 SAY TRAN(vlseg,"@E 999,999.99")       // Valor seguro
		REL_CAB(3)                                     // soma cl/imprime cabecalho
		@ cl,000 SAY LEFT(DTOC(emissao_),6)+RIGHT(DTOC(emissao_),2)// Vencimento
		@ cl,012 SAY tipo+[-]+circ                     // compl 2
		@ cl,017 SAY TRAN(valororig,"@E 999,999.99")  // Valor Contribuicao
		@ cl,027 SAY TRAN(valororig+vlcst,"@E 999,999.99")// Valor Total
		@ cl,041 SAY LEFT(DTOC(emissao_),6)+RIGHT(DTOC(emissao_),2)// Vencimento 2
		IMPCTL(drvpenf)
		@ cl,054 SAY tipo+[-]+circ                     // compl.codigo 2
		IMPCTL(drvtenf)
		@ cl,059 SAY TRAN(valororig,"@E 999,999.99")   // Valor Contribuicao
		@ cl,069 SAY TRAN(valororig+vlcst,"@E 999,999.99")// Valor Total
		REL_CAB(2)                                     // soma cl/imprime cabecalho
		@ cl,001 SAY &drvpcom+rlocpg1+&drvtcom         // Mensagem
		REL_CAB(1)                                     // soma cl/imprime cabecalho
...
msgt="PROCESSAMENTOS DO RELAT¢RIO|PERI¢DICO SEFEX"
ALERTA()
