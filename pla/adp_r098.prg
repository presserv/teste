...
/*
 ...
 \ Programa: ADP_R098.PRG
 \ Data....: 14-05-98
 \ Sistema.: Administradora - PLANO
 \ Funcao..: Vendedor X èrea
...
@ l_s,c_s+16 SAY " VENDEDOR POR èREA "
SETCOLOR(drvcortel)
...
BEGIN SEQUENCE
 DO WHIL ccop<=nucop                               // imprime qde copias pedida
  ...
  qqu038=0                                         // contador de registros
  DO WHIL !EOF()
   #ifdef COM_TUTOR
    ...
    IF INKEY()=K_ESC                               // se quer cancelar
	 ...
	 IF admissao>=M->rde.AND.admissao<=M->rate.AND.(M->rregiao$'000,'+regiao).AND.(M->rvend$'000,'+vendedor)// se atender a condicao...
		...
		@ cl,020 SAY &drvpexp.+'[ '+vendedor+'-'+COBRADOR->nome+' ]'+&drvtexp.// titulo da quebra
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,000 SAY REPL("-",115)
		qb03801=vendedor                               // campo para agrupar 1a quebra
		...
		DO WHIL !EOF() .AND. vendedor=qb03801
		 ...
		 IF admissao>=M->rde.AND.admissao<=M->rate.AND.(M->rregiao$'000,'+regiao).AND.(M->rvend$'000,'+vendedor)// se atender a condicao...
			REL_CAB(1)                                   // soma cl/imprime cabecalho
			@ cl,000 SAY '[ '+regiao+' - '+REGIAO->regiao+' ]'// titulo da quebra
//			REL_CAB(1)                                   // soma cl/imprime cabecalho
			qb03802=vendedor+regiao                      // campo para agrupar 1a quebra
			qqu03802=0                                   // contador de registros
			DO WHIL !EOF() .AND. vendedor=qb03801 .AND. vendedor+regiao=qb03802
			 #ifdef COM_TUTOR
				IF IN_KEY()=K_ESC                          // se quer cancelar
			 #else
				IF INKEY()=K_ESC                           // se quer cancelar
			 ...
			 IF admissao>=M->rde.AND.admissao<=M->rate.AND.(M->rregiao$'000,'+regiao).AND.(M->rvend$'000,'+vendedor)// se atender a condicao...
				...
				qqu03802++                                 // soma contadores de registros
			...
			ENDD
//			REL_CAB(1)                                   // soma cl/imprime cabecalho
			@ cl,117 SAY "* Total: "+TRAN(qqu03802,"@E 999,999")
		 ELSE                                          // se nao atende condicao
			SKIP                                         // pega proximo registro
		 ENDI
		ENDD
		REL_CAB(1)                                     // soma cl/imprime cabecalho
		@ cl,000 SAY "* Quantidade Vendedor "+qb03801+": "+TRAN(qqu03801,"@E 999,999")
		@ ++cl,000 SAY REPL("-",115)
	 ELSE                                            // se nao atende condicao
		SKIP                                           // pega proximo registro
	 ENDI
	ENDD
	REL_CAB(2)                                       // soma cl/imprime cabecalho
	@ cl,000 SAY "*** Quantidade total "+TRAN(qqu038,"@E 999,999")
 ENDD ccop
 EJEC                                              // salta pagina
END SEQUENCE
...
IF cl>maxli .OR. qt=0                              // quebra de pagina
 ...
 @ 1,107 SAY "ADP_R098"                            // c¢digo relat¢rio
